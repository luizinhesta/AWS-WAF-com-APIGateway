...
| **AWS WAF** | Web ACL **regional** `waf-lab-sqli` com 2 regras: `Block-SQLi-Lab` (SQLi match statement) e `Block-Fora-do-Brasil` (geo match, bloqueia origem fora do BR). |
...
AWS WAF   (Web ACL waf-lab-sqli)
  ↓
Block-Fora-do-Brasil  → origem fora do BR? → BLOCK (403)
  ↓ (origem no Brasil)
Block-SQLi-Lab        → padrão de SQLi na query string? → BLOCK (403)
  ↓ (sem SQLi)
HTTP 403 apenas nos casos acima; caso contrário segue para a Lambda
```

O AWS WAF é avaliado **no stage do API Gateway, antes da integração com a Lambda**. Se a origem estiver fora do Brasil, a regra `Block-Fora-do-Brasil` retorna **HTTP 403**. Se a origem for o Brasil mas a query string contiver o padrão de SQLi, a regra `Block-SQLi-Lab` retorna **HTTP 403**. Em ambos os casos a **Lambda não executa** — não há entrada correspondente no CloudWatch Logs nem incremento em `Invocations`. Essa ausência é a principal evidência do laboratório.
...
- **Web ACL:** a mesma `waf-lab-sqli` (regional, associada ao stage `com-waf`).
- **Regra:** `Block-Fora-do-Brasil`.
- **Statement:** Geographic match (correspondência geográfica) com **Negate statement** ativo.
- **Configuração:** país selecionado = **Brazil (BR)**; com o NOT ativo, a regra corresponde quando o país de origem **não** é o Brasil.
- **IP usado:** **Source IP address** (IP de origem da requisição).
- **Ação:** `BLOCK`.

O WAF determina o país pela **geolocalização do IP de origem** (base de dados da AWS). Requisições vindas do Brasil não correspondem à regra e seguem para a inspeção seguinte; requisições de qualquer outro país recebem **HTTP 403**, independentemente da rota ou do conteúdo.

```
Origem no Brasil        →  regra NÃO corresponde  →  segue para a regra SQLi
Origem fora do Brasil   →  regra corresponde       →  BLOCK (403)
```

Como o geo-bloqueio barra a requisição pela **origem**, ele atua antes mesmo de qualquer payload ser inspecionado — por isso, de fora do Brasil, até `/health` retorna 403.

---

## Ordem de avaliação das regras

Ambas as regras têm ação `BLOCK`, então o resultado final não muda com a ordem. Por clareza didática, é comum deixar `Block-Fora-do-Brasil` com **prioridade mais alta** (avaliada primeiro): assim, uma origem estrangeira é barrada antes de qualquer inspeção de SQLi.
...
- **2 regras** (`Block-SQLi-Lab` + `Block-Fora-do-Brasil`): uma demonstra inspeção de conteúdo (SQLi), a outra demonstra controle por origem geográfica (geo match), mantendo o custo baixo.
